...
var log = require('winston-simple').getLogger('DefaultFilterManager');
...
DefaultSessionHandlerManager.prototype.getStartSessionHandlers = function() {
	...
	if (itemArray == null && !this._notFound.start) { // isn't in cache and was never looked for
		...
		if (itemArray == null) this._notFound.start == true; // never will find it, so record this fact so we don't ever look again
	}
...
DefaultSessionHandlerManager.prototype.getEndSessionHandlers = function() {
	...
	if (itemArray == null && !this._notFound.end) { // isn't in cache and was never looked for
		for (var i=0;i<providers.length;i++) {
		...
		if (itemArray == null) this._notFound.end == true; // never will find it, so record this fact so we don't ever look again
	}
