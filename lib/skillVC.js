    ...
    try {
		...
		sv.init(event, context, {
			success : function() {
				...
				// FIXME: needs to call the executor to run the session starts..
				if (event.session.new && sv._skillVCContext.appConfig.sessionHandlerExecutor) {
				...
				if (event.request.type === "IntentRequest" || event.request.type === "LaunchRequest") {
					...
					&& sv._skillVCContext.appConfig.sessionHandlerExecutor) 
				{
					sv._skillVCContext.appConfig.sessionHandlerExecutor.executeEnd(sv._skillVCContext);
				}
				else {
