...
/** @private */
var log = require("winston-simple").setLevels({'all' : 'debug'}).getLogger('SkillVCFactory');
...
SkillVCFactory.createfromDirectory = function(dir) {
	log.verbose('Initializing using Convention-over-Configuration');

	var rootDir = (dir) ? dir : '.'
	log.debug('Using root directory: '+rootDir);

	return new SkillVC({
		'responseManager' 	: ResponseManagerFactory.createHandlebarEnabledByDirectory(path.join(rootDir,'/responses')),
		'filterManager'	: FilterManagerFactory.createByDirectory(path.join(rootDir,'/filters')),
		'intentHandlerManager' : IntentHandlerManagerFactory.createByDirectory(path.join(rootDir,'/intents')),
		'sessionHandlerManager': SessionHandlerManagerFactory.createByDirectory(path.join(rootDir,'/sessionHandlers')),
		'logLevels' 	: {'all' : 'debug'}
	});
}
...
SkillVCFactory.createFromScan = function(files) {
	log.verbose('Initializing using Scanning');
...
SkillVCFactory.createFromConfig = function(config) {
	log.verbose('Not implemented yet');
}
