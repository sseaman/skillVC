...
var log = Logger.getLogger('SkillVCFactory');
...
SkillVCFactory.createfromDirectory = function() {
	Logger.setLevels({'all' : 'debug'});  //FIXME:  bad...

	log.verbose('Initializing using Convention-over-Configuration');

	return new SkillVC({
		'responseManager' 	: ResponseManagerFactory.createHandlebarEnabledByDirectory('./responses'),
		'filterManager'	: FilterManagerFactory.createByDirectory('./filters'),
		'intentHandlerManager' : IntentHandlerManagerFactory.createByDirectory('./intents'),
		'sessionHandlerManager': SessionHandlerManagerFactory.createByDirectory('./sessionHandlers'),
		'logLevels' 	: {'all' : 'debug'}
	});
}
