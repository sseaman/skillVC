...
function DefaultProviderByScanning(files, options) {
	...
	for (var fileIdx=0;fileIdx<files.length;fileIdx++) {
		if (!alreadyLoaded[files[fileIdx]]) {
			...
			providerUtil.addFunctions(loaded, { 'name' : path.parse(files[fileIdx]).base });
			...
			if (isResponse) {
				...
				log.debug("Loading as response with id: " + responseId);
 ...
 */
DefaultProviderByScanning.prototype.getItem = function() {
	return this._items;
};
