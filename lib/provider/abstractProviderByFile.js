...
function AbstractProviderByFile(file, options) {
	...
	if (this._preload) { // this is a blocking call as it is sync
		var processed = this.processItems(this_items, file, this._options);
...
AbstractProviderByFile.prototype.getItem = function(itemId) {
	...
	if (!item && !this._fileLoaded) {
		var processed = this.processItem(this._item, this._file, this._options); // didn't find it and haven't looked for it
		for (var i=0;i<processed.length;i++) {
			this._items[processed[i].itemId] = processed[i].item;
		}
		this._fileLoaded = true;
 ...
 */
AbstractProviderByFile.prototype.processItem = function(itemId, file, options) {
 ...
 */
AbstractProviderByFile.prototype.processItems = function(itemId, file, options) {
	throw new Error('Must be implemented by subclass');
}
