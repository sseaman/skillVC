...
/**
 * FIXME: This is fundamentally broken.  As the loading won't occur until it is needed in may cases,
 * it doesn't determine what intents the intentHandler can actually handle (via getIntentList()) because
 * it won't actually load the intent handler until it's needed, and it is ALWAYS using the filename first.
 * So, if you have an intentHandler that handles an intent that doesn't match it's file name, and it's not 
 * already loaded, it will never find it.
 * AH!!!!
 * It needs to scan ALL intent handlers at the time of call to find the right one...
 */
...
AbstractProviderByAsyncDirectory.prototype.getItem = function(itemId) {
	...
	if (!item && !this._itemNotFound[itemId]) { 
		...
		this.processItem(this._items, this._item, file, this._options);
