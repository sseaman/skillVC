...
function AbstractProviderBySyncDirectory(directory, options) {
	...
	for (var i=0;i<files.length;i++) {
		if (this._filenameFormatter.isValid(files[i])) {
			this.processItem(this._items, path.join(this._directory, files[i]), options);
		}
	}
 ...
 */
AbstractProviderBySyncDirectory.prototype.processItem = function(itemId, file, options) {
 ...
 */
AbstractProviderBySyncDirectory.prototype.processItems = function(items, file, options) {
	throw new Error('Must be implemented by subclass');
};
