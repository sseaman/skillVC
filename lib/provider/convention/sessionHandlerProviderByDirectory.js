...
function SessionHandlerProviderByDirectory(directory, options) {
	...
	this._populate = function(stage, items, fileName, loaded) {
		...
		log.verbose('Loaded session handler '+loaded.getName()+' for '+ stage + ', position '+ position);
	};
...
SessionHandlerProviderByDirectory.prototype.processItems = function(items, fileName, options) {
	...
	providerUtil.addFunctions(this._loaded, { 'name' : path.parse(fileName).base });

	if (svUtil.isFunction(this._loaded.sessionStart)) {
		this._populate('start', this._sessionHandlers.start, fileName, this._loaded);
		items['start'] = this._sessionHandlers.start;
	}
	if (svUtil.isFunction(this._loaded.sessionEnd)) {
		this._populate('end', this._sessionHandlers.end, fileName, this._loaded);
