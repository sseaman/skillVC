 ...
 */
IntentHandlerProviderByFile.prototype.processItem = function(itemId, fileName, options) {
	try {
		...
		if (svUtil.isFunction(this._loaded.getIntentsList)) {
			...
			for (var i=0;i<handledIntents.length;i++) {
				if (handledIntents[i] == itemId) {
					return {'itemId' : handledIntents[i], 'item': this._loaded};
				}
			}
			return null;
		}
		else {
			return {'itemId' : this._filenameFormatter.parse(fileName)[0], 'item': this._loaded};
		}
	}
	catch (err) {
...
IntentHandlerProviderByFile.prototype.processItems = function(items, fileName, options) {
	try {
		...
		if (svUtil.isFunction(this._loaded.getIntentsList)) {
			...
			var processed = [];
			for (var i=0;i<handledIntents.length;i++) {
				processed.push({'itemId' : handledIntents[i], 'item': this._loaded});
			}
