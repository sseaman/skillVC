...
/**
 * Actually fires the calls to the lambda context succeed() or fail(), depending on which method was called 
 * (favors succeed())
 */
ContextWrapper.prototype.fire = function() {
	if (this._resSucceed) this._context.succeed(this._resSucceed);
	else this._context.fail(this._resFail);
};
