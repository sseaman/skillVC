...
SessionHandlerExecutor.prototype.executeStart = function(svContext) {
	for (var i=0;i<this._start.length;i++) {
		log.verbose("Executing Start SessionHandler "+this._start[i].getName());
		this._start[i].sessionStart(svContext.lambda.event, svContext.lambda.context, svContext);
	}
};
...
SessionHandlerExecutor.prototype.executeEnd = function(svContext) {
	for (var i=0;i<this._end.length;i++) {
		log.verbose("Executing End SessionHandler "+this.end[i].getName());
		this._end[i].sessionEnd(svContext.lambda.event, svContext.lambda.context, svContext);
	}
};
