...
FilterChainExecutor.prototype.execute = function(svContext) {
	...
	var filterChainCallback = {
		...
		//		Ugly.. ugly.. ugly.... I'm ashamed of this part and just need to take the time to redo it
		success : function() {
			// do all the pre's till they are done
			if (preIdx < fcm._pre.length) {
				if (svUtil.isFunction(fcm._pre[preIdx].executePre)) {
					log.verbose("Executing Pre filter "+fcm._pre[preIdx].getName());
					fcm._pre[preIdx++].executePre(svContext.lambda.event, svContext.lambda.context, svContext);
				}
				else {
			...
			// now move on to the posts
				...
				if (svUtil.isFunction(fcm._post[postIdx].executePost)) {
					log.verbose("Executing Post filter "+fcm._post[postIdx].getName());
					fcm._post[postIdx++].executePost(svContext.lambda.event, svContext.lambda.context, svContext);
				}
				else {
		...
		failure : function() {
			...
			if (preIdx < fcm._pre.length) {
				if (svUtil.isFunction(fcm._pre[preIdx].executePreOnError)) {
					log.verbose("Executing Pre filter "+fcm._pre[preIdx].getName());
					fcm._pre[preIdx++].executePreOnError(svContext.lambda.event, svContext.lambda.context, svContext);
				}
				else {
					preIdx++; // go to the next registered one
					filterChainCallback.success();
			...
			// now move on to the posts
				...
				if (svUtil.isFunction(fcm._post[postIdx].executePostOnError)) {
					log.verbose("Executing Post filter "+fcm._post[postIdx].getName());
					fcm._post[postIdx++].executePostOnError(svContext.lambda.event, svContext.lambda.context, svContext);
				}
				else {
					postIdx++; // go to the next registered one
					filterChainCallback.success();
